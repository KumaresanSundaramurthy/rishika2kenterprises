...
/**
 * Customer Profile Modal — Statement Tab
 *
 * @var object      $Cust
 * @var array       $Statement  rows from getCustomerStatementData()
...
$obSigned   = ($obType === 'Debit') ? $obVal : -$obVal;
...
$_custName   = htmlspecialchars($Cust->Name ?? '');
$_custMobile = htmlspecialchars((!empty($Cust->CountryCode) ? $Cust->CountryCode . ' ' : '') . ($Cust->MobileNumber ?? ''));
$_custEmail  = htmlspecialchars($Cust->EmailAddress ?? '');
$_custArea   = htmlspecialchars($Cust->Area ?? '');
?>

<!-- ── Date Range Controls ────────────────────────────────────────────────── -->
<div class="px-3 pt-3 d-flex align-items-end gap-2 flex-wrap" id="cpStmtControls">
    <div>
        <label class="form-label mb-1 cp-stmt-label">From Date</label>
        <input type="text" id="cpStmtFrom" class="form-control form-control-sm cp-stmt-input"
               placeholder="Select date" readonly>
    </div>
    <div>
        <label class="form-label mb-1 cp-stmt-label">To Date</label>
        <input type="text" id="cpStmtTo" class="form-control form-control-sm cp-stmt-input"
               placeholder="Select date" readonly>
    </div>
    <button type="button" class="btn btn-sm btn-primary cp-stmt-btn" id="cpStmtApplyBtn">
        <i class="bx bx-filter-alt me-1"></i>Apply
    </button>
    <button type="button" class="btn btn-sm btn-outline-secondary ms-auto cp-stmt-btn" id="cpStmtPrintBtn">
        <i class="bx bx-printer me-1"></i>Print
    </button>
</div>

<!-- ── Printable Statement Document ──────────────────────────────────────── -->
<div class="p-3" id="cpStmtPrintArea">
    ...
    <div class="stmt-bill-to">
        <div class="stmt-bill-label">Bill To</div>
        <div class="stmt-cust-name"><?php echo $_custName; ?></div>
        <?php if ($_custArea): ?>
        <div class="stmt-cust-meta"><?php echo $_custArea; ?></div>
        <?php endif; ?>
        <?php if ($_custMobile): ?>
        <div class="stmt-cust-meta"><?php echo $_custMobile; ?></div>
        <?php endif; ?>
        <?php if ($_custEmail): ?>
        <div class="stmt-cust-meta"><?php echo $_custEmail; ?></div>
        <?php endif; ?>
    </div>
        ...
        <table class="table table-sm table-hover table-bordered mb-0 cp-stmt-table">
            <thead class="table-light">
                <tr>
                    ...
                    <th>Type</th>
                    <th>Ref No</th>
                    <th class="text-end">Debit (<?php echo $Cur; ?>)</th>
                    <th class="text-end">Credit (<?php echo $Cur; ?>)</th>
            ...
            <tbody>
                ...
                <tr class="table-light">
                    ...
                    <td class="text-end fw-semibold <?php echo $obSigned >= 0 ? 'text-success' : 'text-danger'; ?>">
                        <?php echo smartDecimal(abs($obSigned)); ?>
                        <span class="badge bg-label-secondary ms-1 cp-stmt-badge-dr">
                            <?php echo $obSigned >= 0 ? 'Dr' : 'Cr'; ?>
                        </span>
                    </td>
                </tr>
                ...
                <?php foreach ($stmtRows as $row):
                    ...
                    $typeLabels  = ['Invoice' => 'Invoice', 'Receipt' => 'Receipt', 'Return' => 'Return'];
                    $typeBadges  = ['Invoice' => 'primary',  'Receipt' => 'success', 'Return' => 'warning'];
                    $type        = $row['TxType'] ?? '—';
                ?>
                <tr>
                    <td class="px-2">
                    ...
                    <td class="text-end <?php echo $debit > 0 ? 'text-danger' : 'text-muted'; ?>">
                        <?php echo $debit > 0 ? smartDecimal($debit) : '—'; ?>
                    </td>
                    <td class="text-end <?php echo $credit > 0 ? 'text-success' : 'text-muted'; ?>">
                        <?php echo $credit > 0 ? smartDecimal($credit) : '—'; ?>
                    </td>
                    <td class="text-end fw-semibold <?php echo $bal >= 0 ? 'text-success' : 'text-danger'; ?>">
                        <?php echo smartDecimal(abs($bal)); ?>
                        <span class="text-muted cp-stmt-badge-bal"><?php echo $bal >= 0 ? 'Dr' : 'Cr'; ?></span>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php endif; ?>
                ...
                <tr class="table-active fw-bold">
                    ...
                    <td class="text-end <?php echo $closingBalance >= 0 ? 'text-success' : 'text-danger'; ?>">
                        ...
                        <span class="badge ms-1 cp-stmt-badge-dr <?php echo $closingBalance >= 0 ? 'bg-success' : 'bg-danger'; ?>">
                            <?php echo $closingBalance >= 0 ? 'To Collect' : 'To Pay'; ?>
                        </span>
                    </td>
                </tr>
            </tbody>
...
(function () {
    ...
    // ── Apply button ─────────────────────────────────────────────────────────
    document.getElementById('cpStmtApplyBtn').addEventListener('click', function () {
        var from = document.getElementById('cpStmtFrom').value;
        var to   = document.getElementById('cpStmtTo').value;
        if (!from || !to) return;
        var $body = $('#cpTabContent_statement');
        $body.html('<div class="d-flex justify-content-center align-items-center py-5"><div class="spinner-border text-primary" role="status"></div></div>');
        ajaxLoading(0);
        $.ajax({
            url  : '/customers/getCustomerStatementTab',
            type : 'POST',
            data : {
                CustomerUID : custUID,
                FromDate    : from,
    ...
    // ── Print button — open letterhead statement in new window ───────────────
    document.getElementById('cpStmtPrintBtn').addEventListener('click', function () {
        var area = document.getElementById('cpStmtPrintArea');
        if (!area) return;
        var win = window.open('', '_blank', 'width=900,height=700');
        win.document.write('<!DOCTYPE html><html lang="en"><head>');
        win.document.write('<meta charset="UTF-8">');
        win.document.write('<title><?php echo addslashes($_orgBrand ?: $_orgName); ?> &mdash; Customer Statement</title>');
        ...
        win.document.write('.stmt-header-left{flex:1}');
        win.document.write('.stmt-logo{max-height:56px;max-width:150px;object-fit:contain;display:block;margin-bottom:7px}');
        ...
        win.document.write('.text-end{text-align:right}.text-start{text-align:left}');
        win.document.write('.fw-semibold{font-weight:600}.fw-bold{font-weight:700}');
        win.document.write('.text-success{color:#28a745}.text-danger{color:#dc3545}.text-muted{color:#888}');
        win.document.write('.px-2{padding-left:8px}');
        win.document.write('.table-responsive{overflow:auto}');
        ...
        win.document.write('.bg-label-primary{background:#e7e7ff;color:#5c5cff}');
        win.document.write('.bg-label-success{background:#e0f5ea;color:#28a745}');
        win.document.write('.bg-label-warning{background:#fff4e0;color:#d97706}');
        win.document.write('.bg-label-secondary{background:#f0f0f0;color:#666}');
        win.document.write('.bg-success{background:#28a745;color:#fff}');
        win.document.write('.bg-danger{background:#dc3545;color:#fff}');
        win.document.write('.ms-1{margin-left:4px}');
        win.document.write('.cp-stmt-badge-dr,.cp-stmt-badge-bal{font-size:10px}');
        win.document.write('.py-4{padding-top:24px;padding-bottom:24px}');
        win.document.write('table td.text-center{text-align:center}');
