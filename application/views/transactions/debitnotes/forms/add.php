...
<div class="layout-wrapper transactionPage layout-content-navbar">
    <div class="layout-container">
        ...
        <div class="layout-page">
            ...
            <?php $FormAttribute = ['id' => 'addDNForm', 'name' => 'addDNForm', 'autocomplete' => 'off', 'data-csrf' => $this->security->get_csrf_token_name(), 'data-csrf-value' => $this->security->get_csrf_hash()];
                    echo form_open('debitnotes/addDebitNote', $FormAttribute); ?>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-header bg-body-tertiary trans-header-static trans-theme modal-header-center-sticky d-flex justify-content-between align-items-center pb-3">
                            <div class="d-flex flex-wrap align-items-center gap-3" id="transHeaderInfo">
                                <h5 class="modal-title mb-0 ms-2">Create Debit Note</h5>
                                <div class="d-flex align-items-center gap-1">
                                    <div class="input-group w-auto">
                                        ...
                                        function buildDNInitialPrefixSegment($cfg) {
                                            if (!$cfg) return '';
                                            $sep = $cfg->Separator ?? '-';
                                            $parts = [$cfg->Name];
                                            ...
                                            if (!empty($cfg->IncludeFiscalYear)) {
                                                ...
                                                $parts[] = ($cfg->FiscalYearFormat ?? 'SHORT') === 'LONG' ? $fy . '-' . ($fy + 1) : str_pad($fy % 100, 2, '0', STR_PAD_LEFT) . '-' . str_pad(($fy + 1) % 100, 2, '0', STR_PAD_LEFT);
                                            }
                                            return implode($sep, $parts) . $sep;
                                        }
                                        $initialPrefixSeg = buildDNInitialPrefixSegment($defaultPrefixConfig);
                                        $initialNextNumber = ($defaultPrefixConfig && isset($NextNumberMap[(int)$defaultPrefixConfig->PrefixUID])) ? (int)$NextNumberMap[(int)$defaultPrefixConfig->PrefixUID] : 1;
                                        ?>
                                        <select id="transPrefixSelect" name="transPrefixSelect" class="select2 form-select form-select-sm" required>
                                        <?php foreach ($PrefixData as $preData): $isSelected = $preData->IsDefault == 1 ? 'selected' : ''; ?>
                        ...
                        <div class="card-body card-body-form-static p-4">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="mb-2">
                                        <label for="transDate" class="form-label small fw-semibold">Debit Note Date <span class="text-danger">*</span></label>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" id="referenceDetails" name="referenceDetails" class="form-control form-control-sm" placeholder="PO No, Ref No..." maxlength="100" />
                                    </div>
                                </div>
                            </div>
                            <hr/>
                            ...
                            <div class="card-header modal-header-center-sticky p-1 mb-3">
                                <div class="d-flex align-items-center gap-2">
                                    <h5 class="modal-title mb-0"><i class="bx bx-cart-add me-1"></i> Products / Services</h5>
                            ...
                            <div class="row">
                                    ...
                                    <div class="table-responsive">
                                        <table id="billTable" class="table trans-table table-bordered table-sm table-hover align-middle" data-update-delay="300">
                                            <thead class="table-light trans-table-light">
                                                <tr>
                                                    ...
                                                    <th style="width:15%;"><div class="d-flex align-items-center gap-1 justify-content-center"><span class="fw-semibold text-nowrap">Discount on</span><select class="form-select form-select-sm w-auto" id="discApplyFor" name="discApplyFor"><option value="PriceWithTax">Price With Tax</option><option value="UnitPrice">Unit Price</option><option value="NetAmount">Net Amount</option><option value="TotalAmount" selected>Total Amount</option></select></div></th>
                                                    <th style="width:15%;"><div class="text-end"><div class="fw-semibold">Total</div></div></th>
                                                </tr>
                                            </thead>
                                            <tbody id="billTableBody">
                                                <tr class="text-center text-muted"><td colspan="6"><div class="py-4"><i class="bx bx-cart text-primary" style="font-size:2rem;"></i><p class="mt-2 mb-0">No items added yet</p></div></td></tr>
                                            </tbody>
                                        ...
                                        <div class="row mt-1 m-1 p-2">
                                            <div class="d-flex align-items-center justify-content-between px-1">
                                                <div class="col-md-4">
                                                    <label class="form-label fw-semibold mb-0">Apply Discount (%) to all items</label>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-2"><label for="transNotes" class="form-label small fw-semibold">Notes</label><textarea class="form-control" name="transNotes" id="transNotes" rows="2" placeholder="Reason for debit note"></textarea></div>
                                    <div class="mb-2"><label for="transTermsCond" class="form-label small fw-semibold">Terms & Conditions</label><textarea class="form-control" name="transTermsCond" id="transTermsCond" rows="2"></textarea></div>
                                </div>
...
$(function() {
    ...
    if ($form.length) {
        $form.on('submit', function(e) {
            ...
            setFormLoading('#addDNForm', true, action);
            $.ajax({
                url: '/debitnotes/addDebitNote', method: 'POST', data: postData, cache: false,
                success: function(response) {
                    if (response.Error) { setFormLoading('#addDNForm', false); showFormError(response.Message); }
                    else { Swal.fire({ icon: 'success', title: 'Debit Note Saved', text: response.Message || 'Debit note created successfully.', confirmButtonText: 'OK', timer: 3000, timerProgressBar: true }).then(function() { window.location.href = '/debitnotes'; }); }
                },
                error: function() { setFormLoading('#addDNForm', false); showFormError('Server error. Please try again.'); }
            });
        });
