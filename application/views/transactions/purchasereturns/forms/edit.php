    ...
    <div class="layout-container">
        ...
        <div class="layout-page">
            <div class="content-wrapper">
                ...
                <?php
                $isDraftEdit = ($PRData->DocStatus === 'Draft');
                $editPrefixConfig = null;
                if (!empty($PrefixData)) {
                    foreach ($PrefixData as $_pd) { if ((int)$_pd->PrefixUID === (int)$PRData->PrefixUID) { $editPrefixConfig = $_pd; break; } }
                    if (!$editPrefixConfig) $editPrefixConfig = $PrefixData[0];
                }
                function buildPREditPrefixSegment($cfg) {
                ...
                $editTransNumber = $isDraftEdit ? (int)($NextNumberMap[(int)($editPrefixConfig->PrefixUID ?? 0)] ?? 1) : (int)$PRData->TransNumber;
                $editPrefixSeg   = $isDraftEdit ? buildPREditPrefixSegment($editPrefixConfig) : '';
                $FormAttribute = ['id' => 'editPRForm', 'name' => 'editPRForm', 'autocomplete' => 'off', 'data-csrf' => $this->security->get_csrf_token_name(), 'data-csrf-value' => $this->security->get_csrf_hash()];
                echo form_open('purchasereturns/updatePurchaseReturn', $FormAttribute);
                ?>
                <input type="hidden" name="TransUID" value="<?php echo (int)$PRData->TransUID; ?>" />
                <div class="card mb-3">
                    <div class="card-header bg-body-tertiary trans-header-static trans-theme modal-header-center-sticky d-flex justify-content-between align-items-center pb-3">
                        <div class="d-flex flex-wrap align-items-center gap-3" id="transHeaderInfo">
                            <h5 class="modal-title mb-0 ms-2"><?php echo $isDraftEdit ? '' : 'Edit'; ?> Purchase Return</h5>
                            <?php if (!$isDraftEdit && !empty($PRData->UniqueNumber)): ?><span class="trans-form-doc-number"><?php echo htmlspecialchars($PRData->UniqueNumber); ?></span><?php endif; ?>
                            <div class="d-flex align-items-center gap-1">
                                <div class="input-group w-auto <?php echo (!$isDraftEdit ? 'd-none' : ''); ?>">
                                    ...
                                    <?php foreach ($PrefixData as $preData): $isSelected = (int)$preData->PrefixUID === (int)$PRData->PrefixUID ? 'selected' : ''; ?>
                    ...
                    <div class="card-body card-body-form-static p-4">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="mb-2"><label for="transDate" class="form-label small fw-semibold">Return Date <span class="text-danger">*</span></label><div class="input-group input-group-merge"><span class="input-group-text"><i class="icon-base bx bx-calendar"></i></span><input type="text" class="form-control form-control-sm" id="transDate" name="transDate" readonly="readonly" value="<?php echo htmlspecialchars(format_datedisplay($PRData->TransDate, 'Y-m-d')); ?>" required /></div></div>
                                <div><label for="referenceDetails" class="form-label small fw-semibold">Reference</label><input type="text" id="referenceDetails" name="referenceDetails" class="form-control form-control-sm" maxlength="100" value="<?php echo htmlspecialchars($PRData->Reference ?? ''); ?>" /></div>
                            </div>
                        </div>
                        <hr/>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-2"><label for="transNotes" class="form-label small fw-semibold">Notes</label><textarea class="form-control" name="transNotes" id="transNotes" rows="2"><?php echo htmlspecialchars($PRData->Notes ?? ''); ?></textarea></div>
                                <div class="mb-2"><label for="transTermsCond" class="form-label small fw-semibold">Terms & Conditions</label><textarea class="form-control" name="transTermsCond" id="transTermsCond" rows="2"><?php echo htmlspecialchars($PRData->TermsConditions ?? ''); ?></textarea></div>
...
$(function() {
    ...
    if ($form.length) {
        $form.on('submit', function(e) {
            ...
            setFormLoading('#editPRForm', true, action);
            $.ajax({ url: '/purchasereturns/updatePurchaseReturn', method: 'POST', data: formData, processData: false, contentType: false, cache: false,
                success: function(response) { if (response.Error) { setFormLoading('#editPRForm', false); showFormError(response.Message); } else { Swal.fire({ icon: 'success', title: 'Purchase Return Updated', text: response.Message||'Updated successfully.', confirmButtonText: 'OK', timer: 3000, timerProgressBar: true }).then(function() { window.location.href = '/purchasereturns'; }); } },
                error: function() { setFormLoading('#editPRForm', false); showFormError('Server error. Please try again.'); }
            });
        });
