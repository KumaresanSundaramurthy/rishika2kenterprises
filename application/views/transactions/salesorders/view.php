                            ...
                            <div class="modal-content border-0 shadow-lg" style="border-radius:12px;overflow:hidden;">
                                <div class="d-flex align-items-center justify-content-between px-3 py-2 border-bottom bg-white">
                                    <div class="fw-semibold" style="font-size:.92rem;">
                                        <i class="bx bx-file-blank text-primary me-1"></i>Sales Order Preview
                                    </div>
                        ...
                        <div class="modal-dialog modal-xl modal-dialog-scrollable">
                            <div class="modal-content">
                                <div class="modal-header p-3 d-flex justify-content-between align-items-center">
                                    <h6 class="modal-title fw-semibold text-primary mb-0" id="viewSOModalTitle">Sales Order Details</h6>
                                    <div class="gap-2">
...
$(function () {
    ...
    $(document).on('click', '.viewSalesOrder', function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                if (resp.Error) { $('#viewSOModalBody').html('<div class="alert alert-danger m-3">' + resp.Message + '</div>'); }
                else { $('#viewSOModalTitle').text('Sales Order — ' + (resp.Header.UniqueNumber || 'Details')); $('#viewSOModalBody').html(_buildSODetailHtml(resp)); }
            },
            error: function () { $('#viewSOModalBody').html('<div class="alert alert-danger m-3">Failed to load order.</div>'); }
        });
    });
    ...
    $(document).on('click', '.a4PrintSalesOrder', function () {
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                else { window._soLastPrintData = resp; $('#a4PrintPreview').html(_buildA4Html(resp, $('input[name="a4PaperSize"]:checked').val() || 'A4')); }
            }
        });
    });
    $('input[name="a4PaperSize"]').on('change', function () {
        if (window._soLastPrintData) $('#a4PrintPreview').html(_buildA4Html(window._soLastPrintData, $(this).val()));
    });
    $('#a4PrintBtn').on('click', function () {
        ...
        var content = _buildA4Html(window._soLastPrintData, $('input[name="a4PaperSize"]:checked').val() || 'A4', true);
...
function _buildA4Html(resp, size, forPrint) {
    ...
    var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;font-size:12px;background:#404040}' +
        ...
        '<div style="text-align:right"><strong style="font-size:16px">SALES ORDER</strong><br><span style="color:#666">' + _esc(h.UniqueNumber||'—') + '</span><br>' +
        '<span style="color:#666">Date: ' + _esc(h.TransDate||'') + '</span></div></div>' +
        ...
        '</tfoot></table>' + (h.Notes ? '<p style="font-size:11px;color:#666;margin-top:12px"><strong>Notes:</strong> ' + _esc(h.Notes) + '</p>' : '') +
        '</div></body></html>';
...
$(document).on('click', '.thermalPrintSalesOrder', function () {
    var uid = $(this).data('uid');
    _thermalData = null;
...
function _buildThermalHtml(resp, type) {
    ...
    html += '<div class="tp-row fs-6"><span class="tp-bold">Order #: </span><span class="tp-bold">' + _esc(h.UniqueNumber || '—') + '</span></div>';
