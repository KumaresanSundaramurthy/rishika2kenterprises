...
class Debitnotes extends CI_Controller {
    ...
    public function index() {
        try {
            ...
            $this->pageData['ModRowData']    = $this->load->view('transactions/debitnotes/list', ['DataLists' => $allData, 'SerialNumber' => 0, 'JwtData' => $this->pageData['JwtData']], TRUE);
            $this->pageData['ModPagination'] = $this->globalservice->buildPagePaginationHtml('/debitnotes/getDebitNotesPageDetails', $allDataCount, 1, $limit);
            ...
            $this->load->view('transactions/debitnotes/view', $this->pageData);
        } catch (Exception $e) {
            redirect('dashboard', 'refresh');
        }
    }

    public function getDebitNotesPageDetails($pageNo = 0) {
        ...
        try {
            ...
            $rowHtml = $this->load->view('transactions/debitnotes/list', [
                'DataLists'    => $allData,
            ...
            $this->EndReturnData->Pagination     = $this->globalservice->buildPagePaginationHtml('/debitnotes/getDebitNotesPageDetails', $allDataCount, $pageNo, $limit);
    ...
    public function updateDebitNote() {
        $this->EndReturnData = new stdClass();
        try {
            $this->load->model('dbwrite_model');
            $this->dbwrite_model->startTransaction();
            ...
            if ($existing->DocStatus === 'Draft' && !$isDraft) {
                if ($prefixUID <= 0) throw new Exception('Please select a prefix to finalise this debit note.');
                if ($transNumber <= 0) throw new Exception('Transaction number must be greater than 0.');
                $prefixData = $this->transactions_model->getTransactionsPrefixDetails(['Prefix.PrefixUID' => $prefixUID, 'Prefix.OrgUID' => $orgUID]);
                if (empty($prefixData->Data)) throw new Exception('Invalid prefix selected.');
                $prefix   = $prefixData->Data[0];
    ...
    public function duplicateDebitNote() {
        $this->EndReturnData = new stdClass();
        try {
            $this->load->model('dbwrite_model');
            $this->dbwrite_model->startTransaction();
            ...
            $userUID  = $this->pageData['JwtData']->User->UserUID;
            $orgUID   = $this->pageData['JwtData']->User->OrgUID;
            if ($srcUID <= 0) throw new Exception('Invalid debit note.');
            $this->load->model('transactions_model');
            $src = $this->transactions_model->getTransactionById($srcUID, $orgUID, $this->pageModuleUID);
            if (!$src) throw new Exception('Debit Note not found.');
            ...
            $this->dbwrite_model->commitTransaction();
            $this->EndReturnData->Error    = FALSE;
            $this->EndReturnData->Message  = 'Debit Note duplicated as ' . $uniqueNumber . '.';
            $this->EndReturnData->TransUID = $newTransUID;
            $this->EndReturnData->EditURL  = '/debitnotes/edit/' . $newTransUID;
        } catch (Exception $e) {
            $this->dbwrite_model->rollbackTransaction();
            $this->EndReturnData->Error   = TRUE;
            $this->EndReturnData->Message = $e->getMessage();
    ...
    public function updateDebitNoteStatus() {
        $this->EndReturnData = new stdClass();
        try {
            $this->load->model('dbwrite_model');
            $PostData  = $this->input->post();
            ...
            $userUID   = $this->pageData['JwtData']->User->UserUID;
            $orgUID    = $this->pageData['JwtData']->User->OrgUID;
            if ($transUID <= 0) throw new Exception('Invalid debit note.');
    ...
    public function getDebitNoteDetail() {
        ...
        try {
            ...
            if (!$header) throw new Exception('Debit Note not found.');
            ...
            $printThemeResult = $this->organisation_model->getPrintThemeByType($orgUID, 'Debit Note');
            $this->EndReturnData->Error         = FALSE;
    ...
    public function create() {
        try {
            ...
            $this->load->view('transactions/debitnotes/forms/add', $this->pageData);
        } catch (Exception $e) {
            redirect('debitnotes', 'refresh');
        }
    }
    ...
    public function edit($transUID = 0) {
        try {
            ...
            if ($transUID <= 0) redirect('debitnotes');
            ...
            if (!$transData) redirect('debitnotes');
            $transItems = $this->transactions_model->getTransactionItems($transUID, $orgUID);
            $this->pageData['DNData']    = $transData;
            $this->pageData['DNItems']   = $transItems;
            ...
            $this->load->view('transactions/debitnotes/forms/edit', $this->pageData);
        } catch (Exception $e) {
            redirect('debitnotes', 'refresh');
        }
    }
    ...
    private function _saveAttachments($transUID) {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            $storagePath = 'debitnotes/' . $transUID . '/' . $safeName;
    ...
    public function uploadAttachments() {
        ...
        try {
            ...
            if ($transUID <= 0) throw new Exception('Invalid debit note reference.');
