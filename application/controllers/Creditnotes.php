...
class Creditnotes extends CI_Controller {
    ...
    public function index() {
        try {
            ...
            $this->pageData['ModRowData']    = $this->load->view('transactions/creditnotes/list', ['DataLists' => $allData, 'SerialNumber' => 0, 'JwtData' => $this->pageData['JwtData']], TRUE);
            $this->pageData['ModPagination'] = $this->globalservice->buildPagePaginationHtml('/creditnotes/getCreditNotesPageDetails', $allDataCount, 1, $limit);
            ...
            $this->load->view('transactions/creditnotes/view', $this->pageData);
        } catch (Exception $e) {
            redirect('dashboard', 'refresh');
        }
    }

    public function getCreditNotesPageDetails($pageNo = 0) {
        ...
        try {
            ...
            $rowHtml = $this->load->view('transactions/creditnotes/list', [
                'DataLists'    => $allData,
            ...
            $this->EndReturnData->Pagination     = $this->globalservice->buildPagePaginationHtml('/creditnotes/getCreditNotesPageDetails', $allDataCount, $pageNo, $limit);
    ...
    public function updateCreditNote() {
        ...
        try {
            ...
            if ($existing->DocStatus === 'Draft' && !$isDraft) {
                if ($prefixUID <= 0) throw new Exception('Please select a prefix to finalise this credit note.');
                if ($transNumber <= 0) throw new Exception('Transaction number must be greater than 0.');
                $prefixData = $this->transactions_model->getTransactionsPrefixDetails(['Prefix.PrefixUID' => $prefixUID, 'Prefix.OrgUID' => $orgUID]);
                if (empty($prefixData->Data)) throw new Exception('Invalid prefix selected.');
                $prefix   = $prefixData->Data[0];
    ...
    public function duplicateCreditNote() {
        $this->EndReturnData = new stdClass();
        try {
            $this->load->model('dbwrite_model');
            $this->dbwrite_model->startTransaction();
            ...
            $userUID  = $this->pageData['JwtData']->User->UserUID;
            $orgUID   = $this->pageData['JwtData']->User->OrgUID;
            if ($srcUID <= 0) throw new Exception('Invalid credit note.');
            $this->load->model('transactions_model');
            $src = $this->transactions_model->getTransactionById($srcUID, $orgUID, $this->pageModuleUID);
            if (!$src) throw new Exception('Credit Note not found.');
            ...
            $this->dbwrite_model->commitTransaction();
            $this->EndReturnData->Error    = FALSE;
            $this->EndReturnData->Message  = 'Credit Note duplicated as ' . $uniqueNumber . '.';
            $this->EndReturnData->TransUID = $newTransUID;
            $this->EndReturnData->EditURL  = '/creditnotes/edit/' . $newTransUID;
        } catch (Exception $e) {
            $this->dbwrite_model->rollbackTransaction();
            $this->EndReturnData->Error   = TRUE;
            $this->EndReturnData->Message = $e->getMessage();
    ...
    public function updateCreditNoteStatus() {
        ...
        try {
            ...
            $userUID   = $this->pageData['JwtData']->User->UserUID;
            $orgUID    = $this->pageData['JwtData']->User->OrgUID;
            if ($transUID <= 0) throw new Exception('Invalid credit note.');
    ...
    public function getCreditNoteDetail() {
        $this->EndReturnData = new stdClass();
        try {
            ...
            if (!$header) throw new Exception('Credit Note not found.');
            ...
            $printThemeResult = $this->organisation_model->getPrintThemeByType($orgUID, 'Credit Note');
            $this->EndReturnData->Error         = FALSE;
    ...
    public function edit($transUID = 0) {
        try {
            ...
            if ($transUID <= 0) redirect('creditnotes');
            ...
            if (!$transData) redirect('creditnotes');
            $transItems = $this->transactions_model->getTransactionItems($transUID, $orgUID);
            $this->pageData['CNData']    = $transData;
            $this->pageData['CNItems']   = $transItems;
    ...
    private function _saveAttachments($transUID) {
        ...
        for ($i = 0; $i < $fileCount; $i++) {
            ...
            $storagePath = 'creditnotes/' . $transUID . '/' . $safeName;
    ...
    public function uploadAttachments() {
        ...
        try {
            ...
            if ($transUID <= 0) throw new Exception('Invalid credit note reference.');
