    ...
    <div class="layout-container">
        ...
        <div class="layout-page">
            <div class="content-wrapper">
                ...
                <?php
                $isDraftEdit = ($CNData->DocStatus === 'Draft');
                $editPrefixConfig = null;
                if (!empty($PrefixData)) {
                    foreach ($PrefixData as $_pd) { if ((int)$_pd->PrefixUID === (int)$CNData->PrefixUID) { $editPrefixConfig = $_pd; break; } }
                    if (!$editPrefixConfig) $editPrefixConfig = $PrefixData[0];
                }
                function buildCNEditPrefixSegment($cfg) {
                    if (!$cfg) return '';
                    $sep = $cfg->Separator ?? '-'; $parts = [$cfg->Name];
                ...
                $editTransNumber = $isDraftEdit ? (int)($NextNumberMap[(int)($editPrefixConfig->PrefixUID ?? 0)] ?? 1) : (int)$CNData->TransNumber;
                $editPrefixSeg   = $isDraftEdit ? buildCNEditPrefixSegment($editPrefixConfig) : '';
                $FormAttribute = ['id' => 'editCNForm', 'name' => 'editCNForm', 'autocomplete' => 'off', 'data-csrf' => $this->security->get_csrf_token_name(), 'data-csrf-value' => $this->security->get_csrf_hash()];
                echo form_open('creditnotes/updateCreditNote', $FormAttribute);
                ?>
                <input type="hidden" name="TransUID" value="<?php echo (int)$CNData->TransUID; ?>" />
                <div class="card mb-3">
                        ...
                        <div class="d-flex flex-wrap align-items-center gap-3" id="transHeaderInfo">
                            ...
                            <div class="d-flex align-items-center gap-1">
                                <div class="input-group w-auto <?php echo (!$isDraftEdit ? 'd-none' : ''); ?>">
                                    ...
                                    <?php foreach ($PrefixData as $preData): $isSelected = (int)$preData->PrefixUID === (int)$CNData->PrefixUID ? 'selected' : ''; ?>
                                ...
                                <?php if (!$isDraftEdit): ?>
                                <input type="hidden" name="transPrefixSelect" value="<?php echo (int)$CNData->PrefixUID; ?>" />
                                <input type="hidden" name="transNumber" value="<?php echo (int)$CNData->TransNumber; ?>" />
                                <?php endif; ?>
                            </div>
                        </div>
                    ...
                    <div class="card-body card-body-form-static p-4">
                        <div class="card-header modal-header-center-sticky p-1 mb-3"><h5 class="modal-title mb-0"><i class="bx bx-user me-1"></i> Customer Details</h5></div>
                        <div class="row">
                            <div class="col-md-6 border-end pe-3">
                                <div class="d-flex flex-wrap align-items-center gap-2">
                                    ...
                                    <div class="flex-grow-1"><select id="customerSearch" name="customerSearch" class="form-select form-select-sm"></select></div>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="mb-2">
                                    ...
                                    <div class="input-group input-group-merge"><span class="input-group-text"><i class="icon-base bx bx-calendar"></i></span><input type="text" class="form-control form-control-sm" id="transDate" name="transDate" readonly="readonly" value="<?php echo htmlspecialchars(format_datedisplay($CNData->TransDate, 'Y-m-d')); ?>" required /></div>
                                </div>
                                <div><label for="referenceDetails" class="form-label small fw-semibold">Reference</label><input type="text" id="referenceDetails" name="referenceDetails" class="form-control form-control-sm" maxlength="100" value="<?php echo htmlspecialchars($CNData->Reference ?? ''); ?>" /></div>
                            </div>
                        </div>
                        <hr/>
