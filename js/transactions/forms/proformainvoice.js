// ── Proforma Invoice form — init, submit, and sticky bar ─────────────────────
...
var _formModuleUID     = _cfg.moduleUID  || 113;
...
$(function () {
    ...
    // ── Edit mode pre-fill ────────────────────────────────────────────────────
    if (_isEdit) {
        initTransAttachments(_transUID, '/transactions/getAttachments', 113);

        if (_editData.custUID) {
            $('#customerSearch').append(new Option(
                _editData.custName || '',
                _editData.custUID, true, true
            )).trigger('change');
        }
    ...
    // ── Form submit ───────────────────────────────────────────────────────────
    var _formId = _cfg.formId || 'pfForm';
    var $form   = $('#' + _formId);
    if ($form.length) {
...
(function () {
    var _formId   = (_cfg && _cfg.formId)   ? _cfg.formId   : 'pfForm';
