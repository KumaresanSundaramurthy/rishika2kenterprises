function getSalesOrdersDetails(pageNo, rowLimit, filter) {
    ...
    $('.so-tab-count').text('').addClass('d-none');
    $.ajax({
        url: '/salesorders/getSalesOrdersPageDetails/' + pageNo,
        method: 'POST',
        cache: false,
        data: {
            RowLimit:   rowLimit,
        ...
        success: function(response) {
            if (response.Error) {
                ...
                var $active = $('.so-status-tab.active');
                $active.find('.so-tab-count').text(count > 0 ? count : '').removeClass('d-none');
                initTooltips();
            }
        },
        error: function() {
            $(ModulePag).html('<div class="alert alert-danger m-2">Failed to load sales orders.</div>');
        }
    });
}
