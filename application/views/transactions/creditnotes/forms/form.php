...
<?php
$isEdit      = isset($CNData);
$isDraftEdit = $isEdit && ($CNData->DocStatus === 'Draft');
$transUID    = $isEdit ? (int)$CNData->TransUID : 0;
$formId      = 'cnForm';
$formAction  = $isEdit ? 'creditnotes/updateCreditNote' : 'creditnotes/addCreditNote';

$editPrefixConfig = null;
if ($isEdit && !empty($PrefixData)) {
    foreach ($PrefixData as $_pd) {
        if ((int)$_pd->PrefixUID === (int)$CNData->PrefixUID) { $editPrefixConfig = $_pd; break; }
    }
    if (!$editPrefixConfig) $editPrefixConfig = $PrefixData[0];
}

if ($isEdit && !function_exists('buildCNPrefixSegment')) {
    function buildCNPrefixSegment($cfg) {
        if (!$cfg) return '';
        $sep   = $cfg->Separator ?? '-';
        $parts = [$cfg->Name];
...
$editTransNumber = ($isEdit && $isDraftEdit)
    ...
    : ($isEdit ? (int)$CNData->TransNumber : 0);
$editPrefixSeg = ($isEdit && $isDraftEdit) ? buildCNPrefixSegment($editPrefixConfig) : '';
?>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-header bg-body-tertiary trans-header-static trans-theme modal-header-center-sticky d-flex justify-content-between align-items-center pb-3">
                            <div class="d-flex flex-wrap align-items-center gap-3" id="transHeaderInfo">
                                <h5 class="modal-title mb-0 ms-2">
                                    <?php if (!$isEdit): ?>Create Credit Note<?php elseif ($isDraftEdit): ?>Credit Note<?php else: ?>Edit Credit Note<?php endif; ?>
                                </h5>
                                <?php if ($isEdit && !$isDraftEdit && !empty($CNData->UniqueNumber)): ?>
                                    <span class="trans-form-doc-number"><?php echo htmlspecialchars($CNData->UniqueNumber); ?></span>
                                <?php endif; ?>
                                        ...
                                        <div class="input-group w-auto <?php echo (!$isDraftEdit ? 'd-none' : ''); ?>">
                                            ...
                                            <?php try {
                                                ...
                                                foreach ($PrefixData as $preData) {
                                                    $isSelected = (int)$preData->PrefixUID === (int)$CNData->PrefixUID ? 'selected' : '';
                                                ?>
                        ...
                        <div class="card-body card-body-form-static p-4">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <div class="input-group input-group-merge">
                                            ...
                                            <input type="text" class="form-control form-control-sm" id="transDate" name="transDate" readonly="readonly"
                                                value="<?php echo $isEdit ? htmlspecialchars(format_datedisplay($CNData->TransDate, 'Y-m-d')) : format_datedisplay(time(), 'Y-m-d'); ?>"
                                                required />
                                        </div>
                                    </div>
                                    <div>
                                        <label for="referenceDetails" class="form-label small fw-semibold">Reference</label>
                                        <input type="text" id="referenceDetails" name="referenceDetails" class="form-control form-control-sm" maxlength="100"
                                            value="<?php echo $isEdit ? htmlspecialchars($CNData->Reference ?? '') : ''; ?>" />
                                    </div>
                                </div>
                            </div>
                            <hr/>
                            ...
                            <?php $this->load->view('transactions/partials/form_products_add', [
                                ...
                                'transNotesContent'        => $isEdit ? ($CNData->Notes ?? '') : '',
                                'transTermsContent'        => $isEdit ? ($CNData->TermsConditions ?? '') : '',
...
$(function() {
    ...
    <?php if ($isEdit): ?>
    initTransAttachments(<?php echo $transUID; ?>, '/creditnotes/getAttachments');
    <?php endif; ?>

    searchCustomers('customerSearch');
    <?php if ($isEdit && !empty($CNData->PartyUID)): ?>
    ...
    <?php if (!empty($CNData->GlobalDiscPercent) && $CNData->GlobalDiscPercent > 0): ?>
    $('#globalDiscount').val('<?php echo smartDecimal($CNData->GlobalDiscPercent); ?>').trigger('input');
    <?php endif; ?>
    <?php if (!empty($CNData->ExtraDiscount) && $CNData->ExtraDiscount > 0): ?>
    $('#extraDiscount').val('<?php echo smartDecimal($CNData->ExtraDiscount ?? 0); ?>');
    <?php endif; ?>
    <?php if (!empty($CNData->ExtraDiscountType)): ?>
    $('#extDiscountType').val('<?php echo addslashes($CNData->ExtraDiscountType); ?>');
    <?php endif; ?>
    <?php endif; ?>
    ...
    if ($form.length) {
        ...
        $form.on('submit', function(e) {
            ...
            var ajaxUrl = _isEdit ? '/creditnotes/updateCreditNote' : '/creditnotes/addCreditNote';
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.Error) {
                        ...
                        Swal.fire({
                            icon             : 'success',
                            title            : _isEdit ? 'Credit Note Updated' : 'Credit Note Saved',
                            text             : response.Message || (_isEdit ? 'Updated successfully.' : 'Credit note created successfully.'),
