...
$(function () {
    ...
    $(document).on('click', '.viewPurchase', function () {
        ...
        $('#viewPurchaseModalBody').html('<div class="d-flex justify-content-center py-5"><div class="spinner-border text-primary"></div></div>');
        $('#viewPurchaseEditBtn').attr('href', '/purchases/edit/' + uid);
        $.ajax({
            ...
            success: function (resp) {
                if (resp.Error) {
                    $('#viewPurchaseModalBody').html('<div class="alert alert-danger m-3">' + resp.Message + '</div>');
                } else {
                    $('#viewPurchaseModalTitle').text('Purchase Bill — ' + (resp.Header.UniqueNumber || 'Details'));
                    $('#viewPurchaseModalBody').html(_buildPurchDetailHtml(resp));
                }
            },
            error: function () {
                $('#viewPurchaseModalBody').html('<div class="alert alert-danger m-3">Failed to load purchase bill.</div>');
            }
        });
    });
