                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-6 col-md-3">
                            <a href="javascript:void(0);" class="trans-stat-card stat-all active-stat" data-stat-filter="All">
                                <div class="trans-stat-label">All Credit Notes</div>
                                ...
                                <i class="bx bx-credit-card trans-stat-icon"></i>
                            </a>
                        </div>
                        <div class="col-6 col-md-3">
                    ...
                    <div class="card">
                        ...
                        <div class="trans-toolbar">
                            <ul class="nav trans-status-tabs gap-1" id="cnStatusTabs" role="tablist">
                                <li class="nav-item">
                                    <a class="nav-link active cn-status-tab" data-status="All" href="javascript:void(0);">
                                        All <span class="trans-tab-count ms-1"><?php echo $ModAllCount; ?></span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link cn-status-tab" data-status="Issued" href="javascript:void(0);">
                                        Issued <span class="trans-tab-count ms-1 d-none"></span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link cn-status-tab" data-status="Cancelled" href="javascript:void(0);">
                                        Cancelled <span class="trans-tab-count ms-1 d-none"></span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a class="nav-link cn-status-tab" data-status="Draft" href="javascript:void(0);">
                            ...
                            <div class="d-flex align-items-center gap-2 flex-wrap">
                                ...
                                <div class="input-group input-group-sm" style="width:210px">
                                    ...
                                    <input type="text" class="form-control border-start-0" id="searchTransactionData" placeholder="Credit note # or customer...">
                                </div>
                                <div class="dropdown">
                        ...
                        <div class="table-responsive">
                            <table class="table trans-table table-hover MainviewTable mb-0" id="cnTable">
                                <thead>
                                    <tr>
                                        <th style="width:36px">
                                            <div class="form-check mb-0">
                                                <input class="form-check-input table-chkbox cnHeaderCheck" type="checkbox">
                                            </div>
                                        </th>
                                        ...
                                        <th class="col-sortable cursor-pointer user-select-none" data-sort="Number">
                                            Credit Note # <i class="bx bx-sort-alt-2 ms-1 sort-icon" data-col="Number"></i>
                                        </th>
                        ...
                        <!-- Pagination -->
                        <hr class="my-0">
                        <div class="row mx-3 my-2 justify-content-between align-items-center cnPagination" id="cnPagination">
...
const ModulePag    = '.cnPagination';
const ModuleHeader = '.cnHeaderCheck';
const ModuleRow    = '.cnCheck';
...
$(function () {
    ...
    $(document).on('click', '[data-stat-filter]', function () {
        ...
        $('.cn-status-tab[data-status="' + status + '"]').addClass('active');
        Filter.Status = status; PageNo = 1; getCreditNotesDetails();
    });

    $(document).on('click', '.cn-status-tab', function (e) {
        e.preventDefault();
        $('.cn-status-tab').removeClass('active'); $(this).addClass('active');
    ...
    $(document).on('click', '.pageRefresh', function (e) { e.preventDefault(); PageNo = 1; getCreditNotesDetails(); });

    $('#searchTransactionData').on('keyup', debounce(function () {
        Filter.Name = $.trim($(this).val()); PageNo = 1; getCreditNotesDetails();
    }, 400));
    ...
    $(document).on('click', '.date-option', function () {
        ...
        PageNo = 1; getCreditNotesDetails();
    });
    ...
    $(document).on('click', '.cnPagination .page-link', function (e) {
        ...
        if (match) { PageNo = parseInt(match[1]); getCreditNotesDetails(); }
    });

    $(document).on('click', '.cn-status-update', function () {
        var uid = $(this).data('uid'), status = $(this).data('status');
        $.ajax({ url: '/creditnotes/updateCreditNoteStatus', method: 'POST', data: { TransUID: uid, Status: status, [CsrfName]: CsrfToken },
            success: function (resp) { if (resp.Error) { Swal.fire({ icon: 'error', text: resp.Message }); } else { getCreditNotesDetails(); } }
        });
    });

    $(document).on('click', '.deleteCreditNote', function () {
        var uid = $(this).data('uid'), num = $(this).data('num') || '';
        Swal.fire({ title: 'Delete Credit Note?', html: num ? 'Delete <strong>' + num + '</strong>? This cannot be undone.' : 'This cannot be undone.',
            icon: 'warning', showCancelButton: true, confirmButtonText: 'Delete', confirmButtonColor: '#d33' })
            .then(function (r) {
                if (!r.isConfirmed) return;
                $.ajax({ url: '/creditnotes/deleteCreditNote', method: 'POST', data: { TransUID: uid, [CsrfName]: CsrfToken },
                    success: function (resp) { if (resp.Error) { Swal.fire({ icon: 'error', text: resp.Message }); } else { getCreditNotesDetails(); Swal.fire({ icon: 'success', text: resp.Message, timer: 1500, showConfirmButton: false }); } }
                });
            });
    });
    ...
    $(document).on('click', '.duplicateCreditNote', function () {
        ...
        Swal.fire({ title: 'Duplicate Credit Note?', text: 'A new draft copy will be created.', icon: 'question', showCancelButton: true, confirmButtonText: 'Duplicate' })
            .then(function (r) {
                if (!r.isConfirmed) return;
                $.ajax({ url: '/creditnotes/duplicateCreditNote', method: 'POST', data: { TransUID: uid, [CsrfName]: CsrfToken },
                    success: function (resp) {
                        if (resp.Error) { Swal.fire({ icon: 'error', text: resp.Message }); }
                        else { getCreditNotesDetails(); Swal.fire({ icon: 'success', text: resp.Message, showCancelButton: true, confirmButtonText: 'Edit Now', cancelButtonText: 'Stay Here' }).then(function (r2) { if (r2.isConfirmed && resp.EditURL) window.location.href = resp.EditURL; }); }
                    }
                });
            });
    });

    $(document).on('change', '.cnHeaderCheck', function () { $('.cnCheck').prop('checked', $(this).is(':checked')); });
});
...
$(document).on('click', '.thermalPrintCreditNote', function () {
    var uid = $(this).data('uid');
    _thermalData = null;
    ...
    AjaxLoading = 0;
    $.ajax({
        url: '/creditnotes/getCreditNoteDetail', method: 'GET', data: { TransUID: uid },
        success: function (resp) {
            AjaxLoading = 1;
...
function _buildThermalHtml(resp, type) {
    ...
    html += '<div class="tp-row fs-6"><span class="tp-bold">Credit Note #: </span><span class="tp-bold">' + _esc(h.UniqueNumber || '—') + '</span></div>';
    html += '<div class="tp-row"><span>Date: </span><span>' + _esc(h.TransDate) + '</span></div></div>';
    html += '<div class="d-flex justify-content-between align-items-center"><div class="tp-row"><span>Customer: </span><span style="text-align:right;max-width:60%">' + _esc(h.PartyName) + '</span></div>';
