...
<div class="layout-wrapper transactionPage layout-content-navbar">
    <div class="layout-container">
        ...
        <div class="layout-page">
            ...
            <?php
            $isDraftEdit = ($PurchData->DocStatus === 'Draft');
            ...
            function buildPurchEditPrefixSegment($cfg) {
                if (!$cfg) return '';
                $sep    = $cfg->Separator ?? '-';
                $parts  = [$cfg->Name];
            ...
            $editTransNumber = $isDraftEdit ? (int)($NextNumberMap[(int)($editPrefixConfig->PrefixUID ?? 0)] ?? 1) : (int)$PurchData->TransNumber;
            $editPrefixSeg   = $isDraftEdit ? buildPurchEditPrefixSegment($editPrefixConfig) : '';

            $FormAttribute = ['id' => 'editPurchForm', 'name' => 'editPurchForm', 'autocomplete' => 'off', 'data-csrf' => $this->security->get_csrf_token_name(), 'data-csrf-value' => $this->security->get_csrf_hash()];
                echo form_open('purchases/updatePurchase', $FormAttribute);
            ?>

                <input type="hidden" name="TransUID" value="<?php echo (int)$PurchData->TransUID; ?>" />
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-header bg-body-tertiary trans-header-static trans-theme modal-header-center-sticky d-flex justify-content-between align-items-center pb-3">
                            <div class="d-flex flex-wrap align-items-center gap-3" id="transHeaderInfo">
                                <h5 class="modal-title mb-0 ms-2"><?php echo $isDraftEdit ? '' : 'Edit'; ?> Purchase Bill</h5>
                                <?php if (!$isDraftEdit && !empty($PurchData->UniqueNumber)): ?>
                                    <span class="badge bg-label-info fs-6"><?php echo htmlspecialchars($PurchData->UniqueNumber); ?></span>
                                <?php endif; ?>
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <?php endif; ?>
                                <a href="/purchases" class="btn btn-label-danger">Close</a>
                            </div>
                        </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <textarea class="form-control" name="transTermsCond" id="transTermsCond" rows="2"><?php echo htmlspecialchars($PurchData->TermsConditions ?? ''); ?></textarea>
                                    </div>
                                </div>
                                                ...
                                                <div class="d-flex align-items-center justify-content-end">
                                                    ...
                                                    <div class="input-group input-group-merge w-70">
                                                        ...
                                                        <?php foreach ($DiscTypeInfo as $DiscType) { ?>
                                                            <option value="<?php echo $DiscType->Name; ?>" <?php echo ($PurchData->ExtraDiscType === $DiscType->Name) ? 'selected' : ''; ?>><?php echo $DiscType->Symbol; ?></option>
                                                        <?php } ?>
                                                        </select>
                                                        <input class="form-control form-control-sm ps-1 w-30" type="text" id="extraDiscount" name="extraDiscount" min="0" step="0.01" placeholder="Extra Discount" onkeydown="return handleDotOnly(event)" oninput="this.value=this.value.slice(0,this.maxLength); validatePriceInput(this, <?php echo $JwtData->GenSettings->PriceMaxLength; ?>, <?php echo $JwtData->GenSettings->DecimalPoints; ?>)" maxlength="<?php echo $JwtData->GenSettings->PriceMaxLength; ?>" value="<?php echo smartDecimal($PurchData->ExtraDiscAmount ?? 0); ?>">
                                                    </div>
                                                </div>
...
$(function () {
    ...
    <?php if (!empty($PurchData->PartyUID)): ?>
    $('#vendorSearch').append(new Option(
        '<?php echo addslashes($PurchData->PartyName ?? ''); ?>',
        <?php echo (int)$PurchData->PartyUID; ?>, true, true
    )).trigger('change');
    <?php endif; ?>

    transDatePickr('#transDate', false, 'Y-m-d', false, true, true, true, 'd-m-Y');
    transDatePickr('#billDueDate', false, 'Y-m-d', false, false, false, true, 'd-m-Y', '#transDate');
    ...
    // ── Edit Purchase Bill form submit ──────────────────────────
    var $form = $('#editPurchForm');
    if ($form.length) {
        ...
        $form.on('submit', function(e) {
            ...
            setFormLoading('#editPurchForm', true, action);

            $.ajax({
                url    : '/purchases/updatePurchase',
                method : 'POST',
                data   : postData,
                cache  : false,
                success: function(response) {
                    if (response.Error) {
                        setFormLoading('#editPurchForm', false);
