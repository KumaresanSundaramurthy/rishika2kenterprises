...
<div class="layout-wrapper transactionPage layout-content-navbar">
    <div class="layout-container">
        ...
        <div class="layout-page">
            ...
            <?php
            $isDraftEdit = ($SOData->DocStatus === 'Draft');
            ...
            function buildSOEditPrefixSegment($cfg) {
                if (!$cfg) return '';
                $sep    = $cfg->Separator ?? '-';
                $parts  = [$cfg->Name];
            ...
            $editTransNumber = $isDraftEdit ? (int)($NextNumberMap[(int)($editPrefixConfig->PrefixUID ?? 0)] ?? 1) : (int)$SOData->TransNumber;
            $editPrefixSeg   = $isDraftEdit ? buildSOEditPrefixSegment($editPrefixConfig) : '';

            $FormAttribute = ['id' => 'editSOForm', 'name' => 'editSOForm', 'autocomplete' => 'off', 'data-csrf' => $this->security->get_csrf_token_name(), 'data-csrf-value' => $this->security->get_csrf_hash()];
                echo form_open('salesorders/updateSalesOrder', $FormAttribute);
            ?>

                <input type="hidden" name="TransUID" value="<?php echo (int)$SOData->TransUID; ?>" />
                        ...
                        <div class="card-header bg-body-tertiary trans-header-static trans-theme modal-header-center-sticky d-flex justify-content-between align-items-center pb-3">
                            <div class="d-flex flex-wrap align-items-center gap-3" id="transHeaderInfo">
                                ...
                                <?php if (!$isDraftEdit && !empty($SOData->UniqueNumber)): ?>
                                    <span class="badge bg-label-primary fs-6"><?php echo htmlspecialchars($SOData->UniqueNumber); ?></span>
                                <?php endif; ?>
                            ...
                            <div class="row">
                                <div class="col-md-3 trans-right-border">
                                    <div class="mb-2">
                                        ...
                                        <select id="orderType" name="orderType" class="form-select form-select-sm">
                                            <option value="Regular" <?php echo ($SOData->QuotationType === 'Regular' || empty($SOData->QuotationType)) ? 'selected' : ''; ?>>Regular</option>
                                            <option value="Without_GST" <?php echo $SOData->QuotationType === 'Without_GST' ? 'selected' : ''; ?>>Without GST</option>
                                        </select>
                                    </div>
                                ...
                                <div class="col-md-3">
                                    <div class="mb-2">
                                        ...
                                        <div class="input-group input-group-merge">
                                            ...
                                            <input type="text" class="form-control form-control-sm" id="transDate" name="transDate" readonly="readonly" value="<?php echo htmlspecialchars(format_datedisplay($SOData->TransDate, 'Y-m-d')); ?>" required />
                                        </div>
                                    </div>
                                    <div class="mb-2">
                                        ...
                                        <div class="input-group input-group-merge">
                                            ...
                                            <input type="text" class="form-control form-control-sm" id="deliveryDate" name="deliveryDate" readonly="readonly"
                                                value="<?php echo !empty($SOData->ValidityDate) ? htmlspecialchars(format_datedisplay($SOData->ValidityDate, 'Y-m-d')) : ''; ?>" />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <input type="text" id="referenceDetails" name="referenceDetails" class="form-control form-control-sm" placeholder="PO Number, Ref No..." maxlength="100"
                                            value="<?php echo htmlspecialchars($SOData->Reference ?? ''); ?>" />
                                    </div>
                                </div>
                            </div>
                            <hr/>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-2">
                                        ...
                                        <textarea class="form-control" name="transNotes" id="transNotes" rows="2"><?php echo htmlspecialchars($SOData->Notes ?? ''); ?></textarea>
                                    </div>
                                    <div class="mb-2">
                                        <label for="transTermsCond" class="form-label small fw-semibold">Terms & Conditions</label>
                                        <textarea class="form-control" name="transTermsCond" id="transTermsCond" rows="2"><?php echo htmlspecialchars($SOData->TermsConditions ?? ''); ?></textarea>
                                    </div>
                                </div>
                                                ...
                                                <div class="d-flex align-items-center justify-content-end">
                                                    ...
                                                    <div class="input-group input-group-merge w-70">
                                                        ...
                                                        <?php foreach ($DiscTypeInfo as $DiscType) { ?>
                                                            <option value="<?php echo $DiscType->Name; ?>" <?php echo ($SOData->ExtraDiscType === $DiscType->Name) ? 'selected' : ''; ?>><?php echo $DiscType->Symbol; ?></option>
                                                        <?php } ?>
                                                        </select>
                                                        <input class="form-control form-control-sm ps-1 w-30" type="text" id="extraDiscount" name="extraDiscount" min="0" step="0.01" placeholder="Extra Discount" onkeydown="return handleDotOnly(event)" oninput="this.value=this.value.slice(0,this.maxLength); validatePriceInput(this, <?php echo $JwtData->GenSettings->PriceMaxLength; ?>, <?php echo $JwtData->GenSettings->DecimalPoints; ?>)" maxlength="<?php echo $JwtData->GenSettings->PriceMaxLength; ?>" value="<?php echo smartDecimal($SOData->ExtraDiscAmount ?? 0); ?>">
                                                    </div>
                                                </div>
...
$(function () {
    ...
    transDatePickr('#deliveryDate', false, 'Y-m-d', false, false, false, true, 'd-m-Y', '#transDate');
    ...
    if ($form.length) {
        ...
        $form.on('submit', function(e) {
            ...
            $.ajax({
                url    : '/salesorders/updateSalesOrder',
                method : 'POST',
                data   : postData,
                cache  : false,
