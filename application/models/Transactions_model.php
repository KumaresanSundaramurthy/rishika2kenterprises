...
class Transactions_model extends MY_Model {
    ...
    public function getPaymentRow(int $paymentUID, int $orgUID): ?object {
        $this->ReadDb->db_debug = FALSE;
        $this->ReadDb->select('PaymentUID, TransUID, ModuleUID, Amount, PartyType, PartyUID, IsOnAccount, OnAccountAppliedTransUID, OnAccountSourcePaymentUID, IsTransferredToCreditNote, IsCancelled, IsExcessApplied, ExcessSourcePaymentUID, ExcessAmount');
        $this->ReadDb->from('Transaction.PaymentsTbl');
    ...
    public function getPaymentsList(int $limit, int $offset, int $orgUID, array $filter): array {
        ...
        try {
            ...
            $this->ReadDb->select([
                ...
                'P.OnAccountSourcePaymentUID',
                'AppliedT.UniqueNumber AS OnAccountAppliedTransNum',
                'SrcPmt.UniqueNumber AS OnAccountSourcePaymentNum',
            ]);
            $this->ReadDb->from('Transaction.PaymentsTbl AS P');
            $this->ReadDb->join('Global.PaymentTypesTbl AS PT', 'PT.PaymentTypeUID = P.PaymentTypeUID', 'LEFT');
            $this->ReadDb->join('Transaction.TransactionsTbl AS T', 'T.TransUID = P.TransUID AND T.IsDeleted = 0', 'LEFT');
            ...
            $this->ReadDb->join('Organisation.OrgBankAccountsTbl AS BA', 'BA.BankAccountUID = P.BankAccountUID', 'LEFT');
            $this->ReadDb->join('Users.UserTbl AS CrUser', 'CrUser.UserUID = P.CreatedBy', 'LEFT');
            $this->ReadDb->join('Transaction.TransactionsTbl AS AppliedT', 'AppliedT.TransUID = P.OnAccountAppliedTransUID AND AppliedT.IsDeleted = 0', 'LEFT');
            $this->ReadDb->join('Transaction.PaymentsTbl AS SrcPmt', 'SrcPmt.PaymentUID = P.OnAccountSourcePaymentUID AND SrcPmt.IsDeleted = 0', 'LEFT');
    ...
    public function getSRCancelDepsMap(array $transUIDs, array $uniqueNumbers): array {
        ...
        if (!empty($uniqueNumbers)) {
            $this->ReadDb->select('ReferenceNo, COALESCE(SUM(Amount), 0) AS Total');
            $this->ReadDb->from('Transaction.PaymentsTbl');
            $this->ReadDb->where_in('ReferenceNo', $uniqueNumbers);
            $this->ReadDb->where(['PaymentTypeUID' => 0, 'PaymentDirection' => 'In', 'IsDeleted' => 0]);
            $this->ReadDb->group_by('ReferenceNo');
            foreach ($this->ReadDb->get()->result() as $cr) {
                $creditMap[$cr->ReferenceNo] = (float)$cr->Total;
            }
        }
    ...
    public function getReturnedQtyMapForItems(array $transProdUIDs, int $orgUID): array {
        ...
        foreach ($this->ReadDb->get()->result() as $r) {
            $map[(int)$r->SourceTransProdUID] = (float)$r->ReturnedQty;
        }
        return $map;
    }
    ...
    public function getCustomerInvoicesWithReturnableItems(int $customerUID, int $orgUID): array {
        ...
        ";
        $query = $this->ReadDb->query($sql, [(int)$customerUID, (int)$orgUID]);
        return $query ? $query->result() : [];
    }
    ...
    public function getPendingInvoicesForCustomer(int $customerUID, int $orgUID): array {
        ...
        $this->ReadDb->from('Transaction.TransactionsTbl AS Ts');
        $this->ReadDb->where([
            'Ts.PartyUID'  => (int)$customerUID,
            'Ts.PartyType' => 'C',
            'Ts.ModuleUID' => 103,
            'Ts.OrgUID'    => (int)$orgUID,
            'Ts.IsDeleted' => 0,
            'Ts.IsActive'  => 1,
        ]);
    ...
    public function getTransactionStubByToken(string $token): ?object {
        ...
        $this->ReadDb->from('Transaction.TransactionsTbl');
        $this->ReadDb->where(['TransToken' => $token, 'IsDeleted' => 0, 'IsActive' => 1]);
        $this->ReadDb->limit(1);
        return $this->ReadDb->get()->row();
    }
