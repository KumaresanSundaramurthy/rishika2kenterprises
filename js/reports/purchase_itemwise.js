(function () {
    ...
    function _render(rows) {
        var tbody  = document.getElementById('piTableBody');
        var tfoot  = document.getElementById('piTableFoot');
        var footer = document.getElementById('piTblFooter');
        if (!tbody) return;
        ...
        for (var i = 0; i < rows.length; i++) {
            ...
            html += '<tr>' +
                '<td>' + (i + 1) + '</td>' +
                '<td><span class="fw-semibold text-orange">' + _esc(r.DocNo) + '</span></td>' +
        ...
        if (tfoot) {
            document.getElementById('piFtQty').textContent     = _fmtQty(totQty, 2);
            document.getElementById('piFtTaxable').textContent = _fmt(totTaxable);
            document.getElementById('piFtTax').textContent     = _fmt(totTax);
            document.getElementById('piFtNet').textContent     = _fmt(totNet);
            tfoot.classList.remove('d-none');
        }
        if (footer) {
            footer.classList.remove('d-none');
            document.getElementById('piRowCount').textContent  = rows.length + ' line item' + (rows.length !== 1 ? 's' : '');
            document.getElementById('piFooterTax').textContent = _fmt(totTax);
            document.getElementById('piFooterNet').textContent = _fmt(totNet);
        }
    ...
    function _updateStats(rows, totQty, totTax, totNet) {
        var elItems = document.getElementById('piStatItems');
        var elQty   = document.getElementById('piStatQty');
        var elTax   = document.getElementById('piStatTax');
        var elNet   = document.getElementById('piStatNet');
     ...
     */
    function _fetch() {
        var from = (document.getElementById('piFrom') || {}).value || '';
        var to   = (document.getElementById('piTo')   || {}).value || '';
        var tbody = document.getElementById('piTableBody');
        if (!tbody) return;
    ...
    function _initDatePickers() {
        var fromHidden  = document.getElementById('piFrom');
        var toHidden    = document.getElementById('piTo');
        var fromDisplay = document.getElementById('piFromDisplay');
        var toDisplay   = document.getElementById('piToDisplay');
        if (!fromDisplay || !toDisplay || typeof flatpickr === 'undefined') return;

        var initFrom = (typeof _piInitFrom !== 'undefined') ? _piInitFrom : '';
        var initTo   = (typeof _piInitTo   !== 'undefined') ? _piInitTo   : '';
    ...
    function _init() {
        ...
        var applyBtn = document.getElementById('piApplyBtn');
        if (applyBtn) {
