                            ...
                            <div class="col-md-12">
                                <div class="d-flex flex-column flex-md-row justify-content-between align-items-center p-3">
                                    <ul class="nav nav-pills flex-row" role="tablist">
                                        <li class="nav-item">
                                            <a class="nav-link active" role="tab" href="javascript:void(0);">
                                                <i class="bx bx-server me-1"></i> Storage
                                            </a>
                                        </li>
                                    </ul>
...
<div id="storageTypeFilterBox" class="card shadow mp-filterbox p-3" style="z-index:9999;display:none;position:fixed;">
    <?php if (!empty($StorageTypeInfo)): ?>
        <?php echo $this->load->view('storage/storagetypefilter', ['StorageTypeInfo' => $StorageTypeInfo], TRUE); ?>
    <?php endif; ?>
</div>
...
$(function() {
    ...
    $('.SearchDetails').on('input', inputDelay(function() {
        ...
        if (searchText.length >= 3) {
            ...
            if (searchText) Filter['SearchAllData'] = searchText;
            $('#SearchDetails').blur();
    ...
    $(document).on('click', '.DeleteStorage', function(e) {
        ...
        if (GetId) {
            Swal.fire({
                ...
                if (result.isConfirmed) deleteStorage(GetId);
            });
        }
    });
    ...
    $('#btnDelete').click(function(e) {
        ...
        if (SelectedUIDs.length > 0) {
            Swal.fire({
                title: 'Do you want to delete all the selected storage?',
                text: "You won't be able to revert this!",
                icon: "info",
                showCancelButton: true,
                confirmButtonColor: "#d33",
                ...
                if (result.isConfirmed) deleteMultipleStorage();
            });
        }
    });
    ...
    $('#storageForm').on('submit', function(e) {
        ...
        if (myOneDropzone.files.length > 0) {
            ...
            if (!file.isStored) formData.append('UploadImage', myOneDropzone.files[0]);
        }
        const Description = quill.getText().trim();
        if ($.trim(Description) !== '') formData.append('Description', $('#Description .ql-editor').html());
    ...
    $(document).on('click', '.editStorage', function(e) {
        ...
        if (getVal) {
            ...
            if (getDesc) appendToQuill(atob(getDesc), true);
            if (hasValue(getImg)) {
