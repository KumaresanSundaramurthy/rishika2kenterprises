function getPurchaseOrdersDetails(pageNo, rowLimit, filter) {
    ...
    $('.po-tab-count').text('').addClass('d-none');
    $.ajax({
        url: '/purchaseorders/getPurchaseOrdersPageDetails/' + pageNo,
        method: 'POST',
        cache: false,
        data: {
            RowLimit:   rowLimit,
        ...
        success: function(response) {
            if (response.Error) {
                ...
                var $active = $('.po-status-tab.active');
                $active.find('.po-tab-count').text(count > 0 ? count : '').removeClass('d-none');
                initTooltips();
            }
        },
        error: function() {
            $(ModulePag).html('<div class="alert alert-danger m-2">Failed to load purchase orders.</div>');
        }
    });
}
