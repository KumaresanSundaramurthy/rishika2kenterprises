...
<?php
$isEdit      = isset($DNData);
$isDraftEdit = $isEdit && ($DNData->DocStatus === 'Draft');
$transUID    = $isEdit ? (int)$DNData->TransUID : 0;
$formId      = 'dnForm';
$formAction  = $isEdit ? 'debitnotes/updateDebitNote' : 'debitnotes/addDebitNote';

$editPrefixConfig = null;
if ($isEdit && !empty($PrefixData)) {
    foreach ($PrefixData as $_pd) {
        if ((int)$_pd->PrefixUID === (int)$DNData->PrefixUID) { $editPrefixConfig = $_pd; break; }
    }
    if (!$editPrefixConfig) $editPrefixConfig = $PrefixData[0];
}

if ($isEdit && !function_exists('buildDNPrefixSegment')) {
    function buildDNPrefixSegment($cfg) {
        if (!$cfg) return '';
        $sep   = $cfg->Separator ?? '-';
        $parts = [$cfg->Name];
...
$editTransNumber = ($isEdit && $isDraftEdit)
    ...
    : ($isEdit ? (int)$DNData->TransNumber : 0);
$editPrefixSeg = ($isEdit && $isDraftEdit) ? buildDNPrefixSegment($editPrefixConfig) : '';
?>
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-header bg-body-tertiary trans-header-static trans-theme modal-header-center-sticky d-flex justify-content-between align-items-center pb-3">
                            <div class="d-flex flex-wrap align-items-center gap-3" id="transHeaderInfo">
                                <h5 class="modal-title mb-0 ms-2">
                                    <?php if (!$isEdit): ?>Create Debit Note<?php elseif ($isDraftEdit): ?>Debit Note<?php else: ?>Edit Debit Note<?php endif; ?>
                                </h5>
                                <?php if ($isEdit && !$isDraftEdit && !empty($DNData->UniqueNumber)): ?>
                                    <span class="trans-form-doc-number"><?php echo htmlspecialchars($DNData->UniqueNumber); ?></span>
                                <?php endif; ?>
                                        ...
                                        <div class="input-group w-auto <?php echo (!$isDraftEdit ? 'd-none' : ''); ?>">
                                            ...
                                            <?php try {
                                                ...
                                                foreach ($PrefixData as $preData) {
                                                    $isSelected = (int)$preData->PrefixUID === (int)$DNData->PrefixUID ? 'selected' : '';
                                                ?>
                        ...
                        <div class="card-body card-body-form-static p-4">
                                    ...
                                    <div class="mb-2">
                                        ...
                                        <div class="input-group input-group-merge">
                                            ...
                                            <input type="text" class="form-control form-control-sm" id="transDate" name="transDate" readonly="readonly"
                                                value="<?php echo $isEdit ? htmlspecialchars(format_datedisplay($DNData->TransDate, 'Y-m-d')) : format_datedisplay(time(), 'Y-m-d'); ?>"
                                                required />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <input type="text" id="referenceDetails" name="referenceDetails" class="form-control form-control-sm" maxlength="100"
                                            value="<?php echo $isEdit ? htmlspecialchars($DNData->Reference ?? '') : ''; ?>" />
                                    </div>
                                </div>
                            </div>
                            <hr/>
                            ...
                            <?php $this->load->view('transactions/partials/form_products_add', [
                                ...
                                'transNotesPlaceholder'    => 'Reason for debit note',
                                'transNotesContent'        => $isEdit ? ($DNData->Notes ?? '') : '',
                                'transTermsContent'        => $isEdit ? ($DNData->TermsConditions ?? '') : '',
...
$(function() {
    ...
    <?php if ($isEdit): ?>
    initTransAttachments(<?php echo $transUID; ?>, '/debitnotes/getAttachments');
    <?php endif; ?>

    searchVendors('vendorSearch');
    <?php if ($isEdit && !empty($DNData->PartyUID)): ?>
    ...
    <?php if (!empty($DNData->GlobalDiscPercent) && $DNData->GlobalDiscPercent > 0): ?>
    $('#globalDiscount').val('<?php echo smartDecimal($DNData->GlobalDiscPercent); ?>').trigger('input');
    <?php endif; ?>
    <?php if (!empty($DNData->ExtraDiscount) && $DNData->ExtraDiscount > 0): ?>
    $('#extraDiscount').val('<?php echo smartDecimal($DNData->ExtraDiscount ?? 0); ?>');
    <?php endif; ?>
    <?php if (!empty($DNData->ExtraDiscountType)): ?>
    $('#extDiscountType').val('<?php echo addslashes($DNData->ExtraDiscountType); ?>');
    <?php endif; ?>
    <?php endif; ?>
    ...
    if ($form.length) {
        ...
        $form.on('submit', function(e) {
            ...
            var ajaxUrl = _isEdit ? '/debitnotes/updateDebitNote' : '/debitnotes/addDebitNote';
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.Error) {
                        ...
                        Swal.fire({
                            icon             : 'success',
                            title            : _isEdit ? 'Debit Note Updated' : 'Debit Note Saved',
                            text             : response.Message || (_isEdit ? 'Updated successfully.' : 'Debit note created successfully.'),
