function getPurchasesDetails(pageNo, rowLimit, filter) {
    ...
    $('.purch-tab-count').text('').addClass('d-none');
    $.ajax({
        url: '/purchases/getPurchasesPageDetails/' + pageNo,
        method: 'POST',
        cache: false,
        data: {
            RowLimit:   rowLimit,
        ...
        success: function(response) {
            if (response.Error) {
                ...
                var $active = $('.purch-status-tab.active');
                $active.find('.purch-tab-count').text(count > 0 ? count : '').removeClass('d-none');
                initTooltips();
            }
        },
        error: function() {
            $(ModulePag).html('<div class="alert alert-danger m-2">Failed to load purchase bills.</div>');
        }
    });
}
